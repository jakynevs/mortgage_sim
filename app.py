...
def add_client():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO Client (name, dni, email, requested_capital) VALUES (?, ?, ?, ?)",
...
def get_mortgage_sim(dni):
    ...
    if client:
        ...
        if not isinstance(tae, (int, float)) or tae <= 0 or tae > 100:
            return (
                jsonify({"error": "tae must be a positive number and less than 100"}),
                400,
            )
        ...
        if (
            not isinstance(repayment_term, (int))
            or repayment_term <= 0
            or repayment_term > 40
        ):
            return (
                jsonify(
                    {
                        "error": "repayment_term must be a positive number and less than 40 years"
                    }
                ),
                400,
            )
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO MortgageSimulation (client_id, tae, repayment_term, monthly_instalment, total) VALUES (?, ?, ?, ?, ?)",
