...
def add_test_client_with_requested_capital(client, requested_capital):
    ...
    response = client.post('/client', data=json.dumps(test_client_data), content_type='application/json')
...
def calculate_expected_monthly_instalment(tae, repayment_term, requested_capital):
    ...
    monthly_instalment = requested_capital * monthly_interest_rate / (1 - (1 + monthly_interest_rate) ** (-num_payments))
...
def test_mortgage_sim(client):
    ...
    response = client.post(f'/client/mortgage_sim/{test_dni}', data=json.dumps(test_data), content_type='application/json')
    ...
    expected_monthly_instalment, total = calculate_expected_monthly_instalment(test_data['tae'], test_data['repayment_term'], requested_capital)
    assert round(response_json['monthly_instalment'], 2) == round(expected_monthly_instalment, 2)
